...
# Objective function to evaluate the difference between model output and target output
objective_function <- function(param_value, target_output,age_young) {
  # Modify parameter (example: varying transmission rate)
  param_Baseline <- getModelInputs(scenario="Rubella")
  if(age_young == TRUE){
    # Adjust parameters for young age
    param_Baseline$u[c(1,2)] <- rep(param_value, 2)  # Replace with actual indices and values for young age
  } else {
    # Adjust parameters for old age
    param_Baseline$u[c(62,63)] <- rep(param_value, 2)  # Replace with actual indices and values for old age
  }
  ...
  # Calculate the objective (sum of squared differences)
...
# Store the objective function results for each parameter value
...
param_Baseline$u[c(1,2)]
param_Baseline$u[c(62,63)] <- rep(0.7,2)
param_Baseline$u[c(62,63)]
